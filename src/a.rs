...
async fn main() -> io::Result<()> {
    ...
    let encoder_port = 12345;   // Replace with actual Encoder port
    let listen_addr = format!("{}:{}", encoder_ip, encoder_port);

    // Define the IP and port to forward packets to (Decoder)
    let decoder_ip = "192.168.1.2";  // Replace with Decoder IP
    let decoder_port = 54321;        // Replace with Decoder port
    ...
    let mut buf = vec![0u8; 1500]; // Buffer to hold incoming packets
    ...
    loop {
        ...
        let bytes_sent = socket.send_to(&buf[..len], &decoder_addr).await?;
